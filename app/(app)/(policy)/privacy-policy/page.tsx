...
const onThisPageItems = privacyPolicyContent.policySections.map((section) => ({
  id: section.id,
    ...
    <div className="scroll-mt-24 bg-background">
      ...
      <div className="border-b border-border/60">
        <div className="container">
          <div className="grid grid-cols-1 divide-y divide-border/60 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            {privacyPolicyContent.summaryItems.map((item, i) => {
      ...
      <div className="container grid gap-16 py-16 lg:grid-cols-[minmax(0,1fr)_240px] lg:py-24">
        <article className="min-w-0">
          ...
          {/* Policy sections */}
          {privacyPolicyContent.policySections.map((section, i) => (
            <section
              key={section.id}
              id={section.id}
                ...
                <div className="mt-5 w-full overflow-hidden border-y border-border/60">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-border/40">
                      {privacyPolicyContent.dataCategories.map(
                        ([category, examples]) => (
                          <tr
                            key={category}
          ...
          <div className="flex gap-5 border-t border-border/60 pt-8">
            ...
            <p className="text-xs leading-6 text-muted-foreground/50">
              ...
              applications linked from our site. Last reviewed{" "}
        ...
        <aside className={"hidden lg:block"}>
          <div className="sticky top-[calc(var(--header-height)+1.5rem)] self-start">
            ...
            <SuggestedPages pages={privacyPolicyContent.suggestedPages} />
          </div>
        </aside>
      </div>
    </div>
